				...
				$output .= '<div class="table_of_content_wrapper">';
					$output .= '<ol class="mfn_toc_'. $marker_view .'">';
						...
						foreach($mfn_headings as $key => $value){
							...
							$href_parsed = preg_replace('/[^a-zA-Z0-9]/', '-', wp_strip_all_tags(do_shortcode($value)));
							if(substr($href_parsed, -1) === '-') { //if last letter was special sign, remove it
								$href_parsed = substr($href_parsed, 0, -1);
							}
